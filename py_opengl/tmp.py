...
class Tree0:
    ...
    def raycast_hit(self, ray: geometry.Ray3) -> list[T|None]:
        result: list[T|None]= []
    ...
    def foo(self, ab: geometry.AABB3) -> T|None:
        result:T|None= None
        que=[self.root]
        min_dist: float= maths.MAX_FLOAT
        
        p0= ab.center

        while que:
            current= que.pop(0)
            p1= current.aabb.center
            ...
            if current.aabb.perimeter() <= ab.perimeter():
                if current.left != None:
                    que.append(current.left)

                if current.right != None:
                    p2= current.right.aabb.center
                    dist= p0.dist_sqr(p2)
                    if min_dist > dist:
                        que.append(current.right)
            else:
...
class Node1:
    ...
    def _find_closest(self, objs: list[T|None], ray: geometry.Ray3) -> T|None:
        if len(objs) == 0:
            return None
        ...
        for obj in objs:
            t: float= ray.cast_aabb(obj.compute_aabb())
            if t < 0:
                continue
    ...
    def _raycast(self, ray: geometry.Ray3) -> T|None:
        t: float= ray.cast_aabb(self.aabb)
        if t >= 0:
            if not self.children:
                return self._find_closest(self.objs, ray)
            else:
                tt: list[T|None]= []

                for child in self.children:
                    obj: T|None= child._raycast(ray)    
                    if obj != None:
                        tt.append(obj)
                print(tt)
                return self._find_closest(tt, ray)
